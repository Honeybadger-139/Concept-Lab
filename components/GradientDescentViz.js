...
function buildCurve() {
  ...
  for (let i = 0; i <= 80; i++) {
    ...
    const j = cost(w);
    pts.push(`${i === 0 ? "M" : "L"}${wx(w).toFixed(1)},${jy(j).toFixed(1)}`);
  }
  return pts.join(" ");
}
...
export default function GradientDescentViz() {
  ...
  const step = useCallback(() => {
    ...
    setHistory([...newH]);
    setIter(s.iter);
    if (Math.abs(g) < 0.001 || s.iter >= 200) {
      setRunning(false);
      return;
    }
  ...
  const reset = (startW = -1.5) => {
    ...
    setWVal(startW);
    setHistory([{ w: startW, j: cost(startW) }]);
    setIter(0);
  };
  ...
  return (
    <div style={{ fontFamily: "inherit" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        {PRESETS.map((p) => (
          <button key={p.label} onClick={() => { setAlpha(p.alpha); stateRef.current.alpha = p.alpha; reset(-1.5); }}
            style={{ background: alpha === p.alpha ? p.color : "rgba(255,255,255,0.08)", color: "#fff", border: `1.5px solid ${p.color}`, borderRadius: 6, padding: "5px 14px", cursor: "pointer", fontSize: 13, fontWeight: 600, transition: "background .2s" }}>
            {p.label} (α={p.alpha})
          </button>
        ))}
      </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W, display: "block", background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        {[0, 4, 8, 12, 16].map(j => (
          <g key={j}>
            <line x1={PAD.l} y1={jy(j)} x2={W - PAD.r} y2={jy(j)} stroke="rgba(255,255,255,0.06)" strokeWidth={1} />
            <text x={PAD.l - 6} y={jy(j) + 4} textAnchor="end" fill="rgba(255,255,255,0.35)" fontSize={10}>{j}</text>
          </g>
        ))}
        {[-2, 0, 2, 4, 6].map(w => (
          <g key={w}>
            <line x1={wx(w)} y1={PAD.t} x2={wx(w)} y2={H - PAD.b} stroke="rgba(255,255,255,0.06)" strokeWidth={1} />
            <text x={wx(w)} y={H - PAD.b + 16} textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize={10}>{w}</text>
          </g>
        ))}
        {/* Axes labels */}
        <text x={PAD.l - 38} y={H / 2} textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize={11} transform={`rotate(-90,${PAD.l - 38},${H / 2})`}>J(w) — Cost</text>
        <text x={W / 2} y={H - 6} textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize={11}>w — Parameter</text>
        {/* Optimal marker */}
        ...
        {/* Convergence glow */}
        {converged && <circle cx={wx(wVal)} cy={jy(j)} r={16} fill="none" stroke="#22d3ee" strokeWidth={1.5} opacity={0.5}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, margin: "12px 0" }}>
        {[
          ...
          <div key={s.label} style={{ background: "rgba(255,255,255,0.06)", borderRadius: 8, padding: "8px 10px", textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", marginBottom: 2 }}>{s.label}</div>
      ...
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", display: "flex", justifyContent: "space-between" }}>
          <span>Learning rate α</span><span style={{ color: "#facc15", fontFamily: "monospace", fontWeight: 700 }}>{alpha}</span>
        </label>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => setRunning(r => !r)}
          style={{ flex: 1, background: running ? "rgba(244,63,94,0.2)" : "rgba(99,102,241,0.25)", color: "#fff", border: `1.5px solid ${running ? "#f43f5e" : "#6366f1"}`, borderRadius: 8, padding: "9px 0", cursor: "pointer", fontSize: 14, fontWeight: 700, transition: "all .2s" }}>
          {running ? "⏸ Pause" : converged ? "✓ Converged" : "▶ Run"}
        </button>
        <button onClick={() => { reset(-1.5); }}
          style={{ background: "rgba(255,255,255,0.08)", color: "#fff", border: "1.5px solid rgba(255,255,255,0.2)", borderRadius: 8, padding: "9px 16px", cursor: "pointer", fontSize: 14 }}>
          ↺ Reset
        </button>
        <button onClick={() => { if (!running) { stateRef.current.alpha = alpha; const g2 = grad(stateRef.current.w); const nw = stateRef.current.w - alpha * g2; stateRef.current.w = nw; stateRef.current.iter += 1; stateRef.current.history = [...stateRef.current.history, { w: nw, j: cost(nw) }].slice(-60); setWVal(nw); setHistory([...stateRef.current.history]); setIter(stateRef.current.iter); } }}
          style={{ background: "rgba(255,255,255,0.08)", color: "#fff", border: "1.5px solid rgba(255,255,255,0.2)", borderRadius: 8, padding: "9px 16px", cursor: "pointer", fontSize: 14 }}>
          +1 Step
        </button>
      </div>
