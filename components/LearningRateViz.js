...
function miniChart(path, color, iter) {
  ...
  const curX = xi(Math.min(iter, path.length - 1));
  const curY = yj(last.j);

  return { pts, curX, curY, xi, yj, maxJ };
}
  ...
  useEffect(() => {
    if (running) {
      rafRef.current = setInterval(() => {
        setIter(i => {
          if (i >= MAX_ITER) { setRunning(false); return i; }
          return i + 1;
        });
      }, 120);
    } else {
    ...
    <div style={{ fontFamily: "inherit" }}>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <button onClick={() => setRunning(r => !r)}
          style={{ flex: 1, background: running ? "rgba(244,63,94,0.2)" : "rgba(99,102,241,0.25)", color: "#fff", border: `1.5px solid ${running ? "#f43f5e" : "#6366f1"}`, borderRadius: 8, padding: "8px 0", cursor: "pointer", fontSize: 14, fontWeight: 700 }}>
          {running ? "⏸ Pause" : "▶ Run All Three"}
        </button>
        <button onClick={reset}
          style={{ background: "rgba(255,255,255,0.08)", color: "#fff", border: "1.5px solid rgba(255,255,255,0.2)", borderRadius: 8, padding: "8px 18px", cursor: "pointer", fontSize: 14 }}>
          ↺ Reset
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
        {CONFIGS.map((cfg, ci) => {
          ...
          return (
            <div key={ci} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "12px", border: `1.5px solid ${cfg.color}44`, position: "relative" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: cfg.color, marginBottom: 3 }}>{cfg.label}</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 10, lineHeight: 1.4 }}>{cfg.desc}</div>
              <svg viewBox={`0 0 ${C_W} ${C_H}`} style={{ width: "100%", display: "block" }}>
                {/* Grid */}
                {[0, maxJ / 2, maxJ].map(j => (
                  <g key={j}>
                    <line x1={PAD.l} y1={yj(j)} x2={C_W - PAD.r} y2={yj(j)} stroke="rgba(255,255,255,0.07)" strokeWidth={1} />
                    <text x={PAD.l - 4} y={yj(j) + 4} textAnchor="end" fill="rgba(255,255,255,0.25)" fontSize={8}>{j.toFixed(0)}</text>
                  </g>
                ))}
                {[0, 10, 20, 30, 40].map(i => (
                  <text key={i} x={xi(i)} y={C_H - PAD.b + 12} textAnchor="middle" fill="rgba(255,255,255,0.25)" fontSize={8}>{i}</text>
                ))}
                <text x={PAD.l - 26} y={C_H / 2} textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize={9} transform={`rotate(-90,${PAD.l - 26},${C_H / 2})`}>J(w)</text>
                <text x={C_W / 2} y={C_H - 3} textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize={9}>iteration</text>
                {/* Optimal line */}
                ...
                {/* Current point */}
                <circle cx={curX} cy={curY} r={5} fill={cfg.color} stroke="#fff" strokeWidth={1.5}>
                  {running && <animate attributeName="r" values="5;7;5" dur="0.5s" repeatCount="indefinite" />}
                </circle>
              </svg>
              <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
                {[
                  { l: "J(w)", v: Math.min(last.j, 99.99).toFixed(2) },
                  { l: "w", v: last.w.toFixed(3) },
                ].map(s => (
                  <div key={s.l} style={{ background: "rgba(255,255,255,0.06)", borderRadius: 6, padding: "5px 8px", textAlign: "center" }}>
      ...
      <div style={{ marginTop: 12 }}>
        <label style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", display: "flex", justifyContent: "space-between" }}>
          <span>Step through manually</span><span style={{ fontFamily: "monospace", color: "#a5b4fc" }}>iter = {iter} / {MAX_ITER}</span>
        </label>
        <input type="range" min={0} max={MAX_ITER} step={1} value={iter} onChange={e => { setRunning(false); setIter(+e.target.value); }}
