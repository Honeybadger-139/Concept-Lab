...
function sigmoidPath() {
  ...
  for (let i = 0; i <= 100; i++) {
    ...
    pts.push(`${i === 0 ? "M" : "L"}${sz(z).toFixed(1)},${sp(sigmoid(z)).toFixed(1)}`);
  }
  return pts.join(" ");
}
    ...
    <div style={{ fontFamily: "inherit" }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W, display: "block", background: t.bg, borderRadius: 10, border: `1px solid ${t.border}` }}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map(p => (
          <g key={p}>
            <line x1={PAD.l} y1={sp(p)} x2={W - PAD.r} y2={sp(p)}
              stroke={p === 0.5 ? t.gridMid : t.grid} strokeWidth={1} />
            <text x={PAD.l - 6} y={sp(p) + 4} textAnchor="end" fill={t.label} fontSize={10}>{p}</text>
          </g>
        ))}
        {[-6, -4, -2, 0, 2, 4, 6].map(z => (
          <g key={z}>
            <line x1={sz(z)} y1={PAD.t} x2={sz(z)} y2={H - PAD.b} stroke={t.grid} strokeWidth={1} />
            <text x={sz(z)} y={H - PAD.b + 14} textAnchor="middle" fill={t.label} fontSize={10}>{z}</text>
          </g>
        ))}
        ...
        <line x1={sz(zInput)} y1={sp(prob)} x2={sz(zInput)} y2={H - PAD.b}
          stroke={predicted ? "#22d3ee" : "#f43f5e"} strokeWidth={1.5} strokeDasharray="3,3" opacity={0.6} />
        <line x1={PAD.l} y1={sp(prob)} x2={sz(zInput)} y2={sp(prob)}
          stroke={predicted ? "#22d3ee" : "#f43f5e"} strokeWidth={1.5} strokeDasharray="3,3" opacity={0.6} />
      </svg>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
        <button onClick={() => setShowData(d => !d)}
          style={{ background: showData ? "rgba(249,115,22,0.15)" : t.btnBg, border: `1px solid ${showData ? "#f97316" : t.btnBorder}`, color: t.btnText, borderRadius: 6, padding: "6px 14px", cursor: "pointer", fontSize: 13 }}>
          {showData ? "Hide" : "Show"} sample data
        </button>
        <button onClick={() => setZInput(0)}
          style={{ background: t.btnBg, border: `1px solid ${t.btnBorder}`, color: t.btnText, borderRadius: 6, padding: "6px 14px", cursor: "pointer", fontSize: 13 }}>
          Reset z = 0
        </button>
      </div>
