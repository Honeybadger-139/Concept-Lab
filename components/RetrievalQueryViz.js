...
const STEPS = [
  {
    id: "query", icon: "❓", label: "User Query", color: "#6366f1",
...
const INJECTION_STEPS = [
  { id: "load", icon: "📁", label: "Load Documents", color: "#6366f1", detail: "PyPDFDirectoryLoader reads all PDFs from /docs folder" },
    ...
    <div style={{ fontFamily: "inherit" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {[["retrieval", "⚡ Retrieval Pipeline (per query)"], ["injection", "📦 Injection Pipeline (offline)"]].map(([m, label]) => (
          <button key={m} onClick={() => { setMode(m); setActiveStep(-1); setAnimating(false); clearTimeout(timerRef.current); }}
            style={{ flex: 1, background: mode === m ? "rgba(99,102,241,0.25)" : "rgba(255,255,255,0.06)", border: `1.5px solid ${mode === m ? "#6366f1" : "rgba(255,255,255,0.15)"}`, color: "#fff", borderRadius: 8, padding: "8px 12px", cursor: "pointer", fontSize: 13, fontWeight: mode === m ? 700 : 400 }}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        <button onClick={runAnimation} disabled={animating}
          style={{ flex: 1, background: animating ? "rgba(255,255,255,0.05)" : "rgba(99,102,241,0.25)", border: `1.5px solid ${animating ? "rgba(255,255,255,0.1)" : "#6366f1"}`, color: "#fff", borderRadius: 8, padding: "9px 0", cursor: animating ? "default" : "pointer", fontSize: 14, fontWeight: 700, opacity: animating ? 0.5 : 1 }}>
